helpers do
  ...
  def get_user_profile_with(token)
    response = RestClient.post 'https://rpxnow.com/api/v2/auth_info', 'token' => token, 'apiKey' => '<api key>', 'format' => 'json', 'extended' => 'true'
  ...
  def protected!
    response['WWW-Authenticate'] = %(Basic realm="TweetClone") and
