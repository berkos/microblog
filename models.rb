%w(dm-core dm-timestamps dm-aggregates dm-validations open-uri json).each  { |lib| require lib}
DataMapper.setup(:default, ENV['DATABASE_URL'] || 'mysql://root:root@localhost/tweetclone')
...
class User
  ...
  def displayed_statuses
    statuses = []
    statuses += self.statuses.all(:recipient_id => nil, :limit => 10, :order => [:created_at.desc]) # don't show direct messsages
    self.follows.each do |follows| statuses += follows.statuses.all(:recipient_id => nil, :limit => 10, :order => [:created_at.desc]) end if @myself == @user
